...
public class BasicPlayer extends Movable implements Player {
	...
	public BasicPlayer() {
		...
		this.score = 0;
		this.setImages("/images/jeanRobot/Player/herobotWALK/jeanrobot_marche");
		this.direction = 1;
	}
	...
	private void verticalMove2(StateBasedGame game) {
		posjump = false;
		inCol=false;
		for (Plateform p : games.jeanRobot.World.getPlateforms()){
			p.collPlayer(game, this);
		}
		for (Ennemy e : games.jeanRobot.World.getEnemies()){
			e.collPlayer(game, this);
		}
		for (Projectile p : games.jeanRobot.World.getProjectiles()){
			p.collPlayer(game, this);
		}
		if (isTooLow()) {
			loose(game);
	...
	public void lifelost(StateBasedGame game) {
		...
		if (life == 0) {
			game.enterState(4 /* MenuFinPartie */, new FadeOutTransition(),
					new FadeInTransition());
		}
	}
	...
	public void setImages(String path)
	{
		...
		{
			imageIdle[0] = AppLoader.loadPicture(path + "Idle.png");
			imageIdle[1] = AppLoader.loadPicture(path + "Idleg.png");
		}
